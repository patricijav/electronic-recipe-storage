...
function RegisterPage() {
  ...
  async function handleSubmit() {
    if (email.length === 0) {
      ...
      showNotification(
        "error",
        "Vārds nav aizpildīts, bet tas ir obligāts lauks, lūdzu, aizpildiet to!"
      );
      return;
    } else if (lastName.length === 0) {
      showNotification(
        "error",
        "Uzvārds nav aizpildīts, bet tas ir obligāts lauks, lūdzu, aizpildiet to!"
      );
      return;
      ...
      showNotification(
        "error",
        "Parole atkārtoti nav aizpildīta, bet tas ir obligāts lauks, lūdzu, aizpildiet to!"
      );
      return;
      ...
      showNotification(
        "error",
        "Vārdam ir jābūt garumā no 1 līdz 100 simboliem, izlabojiet lauku, lai garums atbilstu!"
      );
      return;
    } else if (lastName.length < 1 || lastName.length > 100) {
      showNotification(
        "error",
        "Uzvārdam ir jābūt garumā no 1 līdz 100 simboliem, izlabojiet lauku, lai garums atbilstu!"
      );
      return;
      ...
      showNotification(
        "error",
        "Parolei atkārtoti ir jābūt garumā no 8 līdz 64 simboliem, izlabojiet lauku, lai garums atbilstu!"
      );
      return;
      ...
      return;
    } else if (!fitsPasswordRules(confirmPassword)) {
      showNotification(
        "error",
        "Parolē atkārtoti jābūt vienam mazam burtam, vienam lielam burtam, vienam ciparam un vienam speciālam simbolam, bet ievadītajā parolē atkārtoti tā nav taisnība, izlabojiet paroli!"
      );
      return;
    } else if (password !== confirmPassword) {
      showNotification(
        "error",
        "Paroles nesakrīt, pārbaudiet ievadītās paroles!"
      );
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showNotification(
          "error",
          "Jau eksistē lietotājs ar šādu e-pastu, pamēģiniet reģistrēties ar citu e-pastu, vai pieslēdzieties ar esošo!"
        );
  ...
  return (
    <PageContainer>
      <Header title="Reģistrēties" linkText={null} />
      <Content>
        <LeftColumn>
          <div>
            <ModifiedHeading>E-pasts *</ModifiedHeading>
            <NiceInput
              type="text"
          ...
          <div>
            <ModifiedHeading>Vārds *</ModifiedHeading>
            <NiceInput
              type="text"
          ...
          <div>
            <ModifiedHeading>Uzvārds *</ModifiedHeading>
            <NiceInput
              type="text"
          ...
          <div>
            <ModifiedHeading>Parole *</ModifiedHeading>
            <NiceInput
              type="password"
          ...
          <div>
            <ModifiedHeading>Parole atkārtoti *</ModifiedHeading>
            <NiceInput
              type="password"
          ...
          <HeavyButton title="Reģistrēties" onClick={handleSubmit} />
        </LeftColumn>
        <RightColumn>
          <HeavyButton
            to="/login"
...
const ModifiedHeading = styled.h3`
  margin: 0;
`;
